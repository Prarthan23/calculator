...
const Distance2DCalculator = () => {
  ...
  const calcTitle = "Distance 2d Calculator"
  const inputData={x1: parseFloat(x1), y1: parseFloat(y1), x2:parseFloat(x2), y2:parseFloat(y2)}
  ...
  return (
    <div className='h-full w-full flex justify-center items-start lg:items-center relative'>
      ...
      <div className='flex mt-4 flex-col h-auto w-80 border border-gray-800 justify-between items-start p-3 bg-gray-100 shadow-lg shadow-gray-400 rounded-lg gap-5 lg:w-[450px]'>
        <h2 className='text-2xl text-center font-semibold tracking-tight'>2D Distance Calculator</h2>
        <input
          type="number"
          value={x1}
