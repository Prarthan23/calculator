...
const ConeCalculator = () => {
  ...
  const calculateCone = () => {
    ...
    const calculatedVolume = (Math.PI * r * r * h) / 3;
    const slantHeight = Math.sqrt(r * r + h * h);
    const calculatedSurfaceArea = Math.PI * r * (r + slantHeight);
  ...
  const calcTitle = "Cone Calculator"
  ...
  return (
    <div className='h-full w-full flex justify-center items-start lg:items-center relative'>
      ...
      <div className='flex mt-4 flex-col h-auto w-80 border border-gray-800 justify-between items-start p-3 bg-gray-100 shadow-lg shadow-gray-400 rounded-lg gap-5 lg:w-[450px]'>
        <h2 className='text-2xl text-center font-semibold tracking-tight'>Cone Calculator</h2>
        <input
          type="number"
          value={radius}
        ...
        <div className='flex w-full justify-between items-center'>
          <button onClick={calculateCone} className='bg-red-700 text-xl uppercase py-1 px-8 text-white rounded-md'>
            Calculate
          </button>
